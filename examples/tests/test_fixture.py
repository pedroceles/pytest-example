...
class TestFixtureMethodVsFunction2:
    ...
    )  # This tells pytest to expect this test to fail
...
class TestFixtureMethodVsFunction3:
    ...
    @pytest.mark.xfail(
        reason="Fixture not injected, but fixture function name is. Will raise AssertionError"
    )
    def test_forget_to_inject_function_fixture(self):
        assert func_value_of_one == 1

    @pytest.mark.xfail(
        reason="Fixture not injected, fixture is method. Will raise NameError"
    )
    def test_forget_to_inject_method_fixture(self):
        assert name_of_test == "test-1"  # noqa
...
class TestScopeDependency:
    ...
    def test_passes_due_to_ok_dependency(
        self, function_scoped_depending_on_class_scoped
    ):
    ...
    def test_fails_due_to_wrong_feature_dependency(
        self, class_scoped_depending_on_function_scoped
    ):
